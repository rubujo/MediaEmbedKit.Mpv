...
本資料夾包含 MediaEmbedKit.Mpv 的 Windows x64 桌面範例。範例用於示範 runtime 準備、播放器初始化、事件輸出、播放控制、AirSpace 行為與資源釋放。
...
- 啟動時呼叫 `SampleRuntime.InstallOrUpdateAsync()` 準備 `libmpv-2.dll`、`yt-dlp.exe` 與 `deno.exe`。
