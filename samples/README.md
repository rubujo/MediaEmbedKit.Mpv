...
本資料夾包含 MediaEmbedKit.Mpv 的 Windows 範例（x64 / ARM64，執行階段輔助工具依目前處理序架構自動選擇對應資產）。範例用於示範執行階段準備、播放器初始化、事件輸出、播放控制、AirSpace 行為與資源釋放。
...
- Console 範例明確呼叫執行階段輔助工具後建立 `MpvPlayer`，用於展示核心 API 最小生命週期。
- 範例為了展示 yt-dlp 後處理、FFmpeg / FFprobe 診斷與完整 runtime，會明確啟用 yt-dlp/FFmpeg-Builds GPL 二進位檔；散發應用程式前請依 [`docs/RUNTIME_ASSETS.md`](../docs/RUNTIME_ASSETS.md) 確認授權義務。
...
- yt-dlp 與 Deno 診斷會透過程式庫的外部處理序執行器接收 stdout/stderr；FFmpeg 與 FFprobe 由執行階段輔助工具作為 yt-dlp 附帶工具準備。
