...
public sealed class MpvOpenGlDrmDisplayOptions
{
    ...
    /// </summary>
    /// <param name="fileDescriptor">DRM 檔案描述元；無效時可使用 -1。</param>
    /// <param name="crtcId">目前使用的 CRTC 識別碼。</param>
    /// <param name="connectorId">目前使用的連接器識別碼。</param>
    ...
    /// 取得 DRM 檔案描述元。
    /// </summary>
    /// <value>DRM 檔案描述元；無效時可使用 -1。</value>
    public int FileDescriptor { get; private set; }

    /// <summary>
    /// 取得目前使用的 CRTC 識別碼。
    /// </summary>
    /// <value>CRTC 識別碼。</value>
    ...
    /// </summary>
    /// <value>DRM atomic request 指標的指標；不使用時為 <see cref="IntPtr.Zero"/>。</value>
    public IntPtr AtomicRequestPointer { get; set; }

    /// <summary>
    /// 取得或設定 DRM render node 檔案描述元。
    /// </summary>
    /// <value>DRM render node 檔案描述元；無效時可使用 -1。</value>
