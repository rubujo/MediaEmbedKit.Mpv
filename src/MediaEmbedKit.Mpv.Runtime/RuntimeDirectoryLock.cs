...
/// 避免兩個應用實例 / 並行 CI 共用同一執行階段資料夾並發寫壞 libmpv-2.dll。
/// </summary>
/// <remarks>
/// <para>
...
/// </para>
/// <para>
/// <strong>非 recursive：同 process 內若已持有此 lock，再次 acquire 同一 path 會 死結</strong>。
/// 呼叫端必須確保只在 最外層 public 入口 取一次，nested 輔助工具不再取。
/// 同 process 內進一步的執行緒互斥由各自 輔助工具的 處理序內鎖定 處理
...
internal sealed class RuntimeDirectoryLock : IDisposable
{
    ...
    /// <summary>
    /// 在指定執行階段資料夾上取得 cross-process 互斥鎖。被其他 process 占用時等待，
            ...
            catch (IOException)
            {
                if (DateTimeOffset.UtcNow >= deadline)
                {
                    throw new TimeoutException(
                        "等待執行階段資料夾鎖（" + lockPath + "）逾時 " + effectiveTimeout +
