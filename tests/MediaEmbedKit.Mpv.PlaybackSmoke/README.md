...
可用範例名稱為 `WinForms`、`WPF`、`Avalonia`、`WinUI`、`MAUI` 或 `all`。第一次執行可能需要下載 Windows x64 / ARM64 執行階段資產。
...
指定 `--runtime-directory` 時，測試器會在啟動 GUI sample 前先準備共用執行階段資料夾；若下載遇到暫時性 HTTP 或代理錯誤，會在 console 輸出失敗原因並重試。`--sample-root` 可指定包含 `samples` 資料夾的根目錄，供乾淨使用端 sample 驗證腳本重用同一個播放檢查器。
