...
本專案目前僅列入已完成基本建置、執行階段來源與範例驗證的 Windows x64 / ARM64 目標。未列入本文件者不提供支援承諾。

## 平台支援判準

一個平台會被列入下方支援矩陣，前提是同時滿足三項條件：

1. **C# UI 框架 可在該平台執行**：至少有一個本專案目前支援的 UI 套件（WinForms / WPF / Avalonia / WinUI 3 / .NET MAUI）在該平台可實際執行。
2. **libmpv 有可程式化取得的 runtime**：該平台有公開、可散佈的 libmpv 二進位資產，且至少滿足以下其一：
   - 有 GitHub Releases 提供穩定命名的二進位資產（本專案執行階段輔助工具直接下載與驗證）；或
   - 有作業系統官方套件管理器套件，使用者預期會自行安裝（本專案不提供執行階段輔助工具，僅以 `DllImport` 名稱解析機制接上）。
...
| Windows ARM64 | WinForms / WPF / Avalonia / WinUI 3 / .NET MAUI Windows | 支援（CI 建置 smoke；待物理機 GUI 播放驗證） | `libmpv-2.dll`（`mpv-dev-aarch64-*.7z`） |

### Windows ARM64 注意事項

- `.NET 10` 在 ARM64 為原生 first-class 支援。
- GitHub Actions 使用 `windows-11-arm` hosted runner 對 ARM64 方案進行建置 smoke；此 runner 目前屬 GitHub 公開預覽，不能取代實體 ARM64 裝置上的 GUI 播放驗證。
...
libmpv 兩個架構皆由 [shinchiro/mpv-winbuild-cmake](https://github.com/shinchiro/mpv-winbuild-cmake/releases) 與 [zhongfly/mpv-winbuild](https://github.com/zhongfly/mpv-winbuild/releases) 提供，命名規範完全相同；既有 `ProviderFallbackOrder` 機制在兩個架構上行為一致。**預設 `Provider = Zhongfly` + `ProviderFallbackOrder = [Shinchiro]`** —— zhongfly 是兩家中唯一同時提供 LGPL 與 GPL libmpv 建置版的來源；shinchiro 只發 GPL。詳見 [`RUNTIME_ASSETS.md`](RUNTIME_ASSETS.md#libmpv-授權版本選擇gpl-vs-lgpl)。

## 驗證狀態

Windows 發佈前驗證以本機發行檢查閘門為準：
...
核心 API 測試、原生整合測試、NuGet 套件內容、乾淨使用端建置、ConsoleMinimal 播放與 GUI 使用端播放都應在發佈前通過。WinForms、WPF、Avalonia、WinUI 3 與 MAUI Windows 範例需以 YouTube 測試網址播放到指定秒數後正常關閉。
...
Windows 執行階段輔助工具目前支援 `libmpv-2.dll`、`yt-dlp.exe`、`deno.exe`、`ffmpeg.exe` 與 `ffprobe.exe` 的同層配置，並依目前處理序架構自動選擇對應的 x64 / ARM64 資產。FFmpeg 來源限定為 yt-dlp `FFmpeg-Builds` Windows GPL 建置版；NuGet 套件不包含任何第三方執行階段二進位檔。
