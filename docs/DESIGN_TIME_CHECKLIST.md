...
Visual Studio 2022/2026 的可視 XAML Designer 僅支援 WPF 與 UWP；WinUI 3 與 .NET MAUI 沒有靜態 designer surface，只能透過 XAML Live Preview 或 XAML Hot Reload 在 runtime 觀察 UI。Avalonia 透過官方 VS extension 提供 previewer。WinForms 設計工具同時涵蓋 .NET Framework 與 .NET 10。本檢查表涵蓋上述所有設計時情境（含 IntelliSense / XAML 編輯器載入），凡是會「載入控制項組件」的時機都適用，不限於拖放編輯。
...
4. 在 XAML Hot Reload / Live Preview 確認（WinUI 3 / MAUI）：
   - 啟動 `samples/WinUISample` 與 `samples/MauiSample`，於 runtime 開啟 XAML Live Preview，修改 `MainWindow.xaml` / `MainPage.xaml` 觀察 Hot Reload 行為，確認沒有 runtime 重新初始化原生播放器。
5. 關閉設計工具與 Hot Reload session 後重新建置方案，確認沒有設計階段殘留檔案或鎖定組件造成建置失敗。

## 失敗處理

- 若設計工具嘗試載入 `libmpv-2.dll`，應檢查對應控制項的 design mode 判斷。
- 若 WinUI 或 MAUI 在 XAML 編輯器 / Hot Reload 載入時建立 HWND 後端，應檢查 handler 連線流程是否在設計模式中提前返回。
