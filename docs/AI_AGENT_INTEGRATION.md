...
| `.github/copilot-instructions.md` | GitHub Copilot 系統指令；以 `tools/Sync-CopilotInstructions.ps1` 自動 inline 同步 `AGENTS.md`。 |
| `.codex/skills/*/SKILL.md` | OpenAI Codex CLI 專案 skill 發現檔。 |
...
| --- | :-: | :-: | :-: | :-: |
| `AGENTS.md` 原生讀取 | 透過 `CLAUDE.md` `@AGENTS.md` | 透過 `GEMINI.md` `@AGENTS.md` + `.gemini/settings.json` | ❌ 不支援 `@import`；採 inline 同步 | ✅ 原生 |
| Skill 發現 | `.claude/skills/` | ❌ 無 skill 系統 | ❌ 無 skill 系統 | `.codex/skills/`（自訂） |
| 工具呼叫前 / 後 hooks | ✅ `.claude/settings.json` | ❌ 不支援 | ❌ 不支援 | ❌ 不支援 |
| 跨 session memory | ✅ `~/.claude/projects/.../memory/` | 部分 | ❌ | 部分 |
| 共用 skill playbook (`docs/ai/skills/*.md`) | ✅ | ✅（透過 AGENTS.md 引用） | ✅（inline 後可看到 skill 路徑） | ✅ |

> Copilot 因為不支援 `@import`，**必須** inline 完整內容；其他三個工具走 `@AGENTS.md` 或原生讀取，內容只維護一份。Hooks 與 memory 為 Claude Code 專屬，無法跨工具實作。
